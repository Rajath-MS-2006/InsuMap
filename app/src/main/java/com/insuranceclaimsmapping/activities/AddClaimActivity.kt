...
class AddClaimActivity : AppCompatActivity() {
    ...
    private val imagePickerLauncher = registerForActivityResult(
        ActivityResultContracts.GetContent()
    ) { uri -> uri?.let { photoUri = it; processImageWithAI(it) } }
    ...
    private fun proceedWithSubmit(role: String, patientIdInput: String, name: String, hospital: String, amount: String, description: String) {
        if (role == "HOSPITAL" && patientIdInput.isNotEmpty() && resolvedPatientUid == null) {
            firebaseHelper.getUserIdByCustomId(patientIdInput, { patientUid: String? ->
                ...
                submitClaim(null, name, hospital, amount, description, emptyList(), patientUid, patientIdInput)
            }, { e: Exception ->
                if (isFinishing || isDestroyed) return@getUserIdByCustomId
                Toast.makeText(this, "Error finding Patient ID: ${e.message}", Toast.LENGTH_SHORT).show()
            })
        } else {
            submitClaim(null, name, hospital, amount, description, emptyList(), if (role == "PATIENT") FirebaseAuth.getInstance().currentUser?.uid else resolvedPatientUid, if (role == "PATIENT") null else patientIdInput)
        }
    }
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            try {
                val bitmap = if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.P) {
                    android.graphics.ImageDecoder.decodeBitmap(android.graphics.ImageDecoder.createSource(contentResolver, uri)) { decoder, _, _ ->
                ...
                withContext(Dispatchers.Main) {
                    ...
                    if (result.items.isNotEmpty() && result.hospitalName.isNotEmpty()) {
                        submitClaim(uri.toString(), result.patientName, result.hospitalName,
                            ...
                            result.items.joinToString(", ") { it.description }, result.items, null, binding.etPatientId.text.toString().trim())
                    } else {
                        ...
                        if (it.pageCount > 0) {
                            ...
                            withContext(Dispatchers.Main) {
                                ...
                                if (result.items.isNotEmpty()) {
                                    submitClaim(uri.toString(), result.patientName, result.hospitalName, result.items.sumOf { it.amount }.toString(), result.items.joinToString { it.description }, result.items, null, binding.etPatientId.text.toString().trim())
    ...
    private fun submitClaim(billUrl: String?, name: String, hospital: String, amount: String, description: String, items: List<com.insuranceclaimsmapping.models.BillItem>, patientId: String?, customPatientId: String?) {
        ...
        val claim = Claim(
            ...
            customPatientId = if (role == "PATIENT") PrefManager(this).getCustomId() ?: "" else (customPatientId ?: ""),
