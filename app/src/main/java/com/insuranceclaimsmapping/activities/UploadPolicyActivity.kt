...
class UploadPolicyActivity : AppCompatActivity() {
        ...
        firebaseHelper.getPolicyHistory(insurerId, { historyList: List<Policy> ->
            ...
            AlertDialog.Builder(this)
                ...
                .setItems(items as Array<CharSequence>) { _, idx ->
                    showPolicyDialog("Version ${historyList[idx].version}", historyList[idx].coverageDetails, insurerId, historyList[idx].version)
                }
    ...
    private fun showPolicyDialog(title: String, details: String, insurerIdToDelete: String? = null, versionToDelete: Int? = null) {
        ...
        if (insurerIdToDelete != null && versionToDelete != null) {
            builder.setNegativeButton("Delete") { _, _ ->
                firebaseHelper.deletePolicyHistory(insurerIdToDelete, versionToDelete, {
                    if (!isFinishing && !isDestroyed) {
                        Toast.makeText(this, "Policy version deleted", Toast.LENGTH_SHORT).show()
                    }
                }, { e ->
                    if (!isFinishing && !isDestroyed) {
